...
(function ( $, window, document, undefined ) {
    'use strict';
    var themesmake_document = $(document);
    ...
    var ATSORTABLE = function () {
        var repeaters = $('.themesmake-repeater');
        repeaters.sortable({
            orientation: "vertical",
            items: '> .repeater-table',
            placeholder: 'themesmake-sortable-placeholder',
    ...
    var ATREPEATER =  function (){
       themesmake_document.on('click','.themesmake-add-repeater',function (e) {
            e.preventDefault();
            var add_repeater = $(this),
                repeater_wrap = add_repeater.closest('.themesmake-repeater'),
                code_for_repeater = repeater_wrap.find('.themesmake-code-for-repeater'),
                total_repeater = repeater_wrap.find('.themesmake-total-repeater'),
            ...
            var repeater_inside = new_html_object.find('.themesmake-repeater-inside');
       ...
       themesmake_document.on('click', '.themesmake-repeater-top, .themesmake-repeater-close', function (e) {
            e.preventDefault();
            var accordion_toggle = $(this),
                repeater_field = accordion_toggle.closest('.repeater-table'),
                repeater_inside = repeater_field.find('.themesmake-repeater-inside');
       ...
       themesmake_document.on('click', '.themesmake-repeater-remove', function (e) {
            e.preventDefault();
            var repeater_remove = $(this),
                repeater_field = repeater_remove.closest('.repeater-table'),
                repeater_wrap = repeater_remove.closest('.themesmake-repeater');

            repeater_field.remove();
            ATREFRESHVALUE(repeater_wrap);
        });

       themesmake_document.on('change', '.themesmake-select', function (e) {
            e.preventDefault();
            var select = $(this),
                repeater_inside = select.closest('.themesmake-repeater-inside'),
                postid = repeater_inside.find('.themesmake-postid'),
                repeater_control_actions = repeater_inside.find('.themesmake-repeater-control-actions'),
   ...
   themesmake_document.ready( function() {
       themesmake_document.on('widget-added widget-updated, panelsopen', function( event, widgetContainer ) {
            ATSORTABLE();
        });
         ...
         */
       themesmake_document.on('click', '.themesmake-icons-wrapper .single-icon', function() {
            var single_icon = $(this),
                at_customize_icons = single_icon.closest( '.themesmake-icons-wrapper' ),
            ...
            at_customize_icons.find('.themesmake-icon-value').val( icon_value );
            at_customize_icons.find('.icon-preview').html('<i class="' + icon_display_value + '"></i>');
            at_customize_icons.find('.themesmake-icon-value').trigger('change');
        });

       themesmake_document.on('click', '.themesmake-icons-wrapper .icon-toggle ,.themesmake-icons-wrapper .icon-preview', function() {
            var icon_toggle = $(this),
                at_customize_icons = icon_toggle.closest( '.themesmake-icons-wrapper' ),
       ...
       themesmake_document.on('keyup', '.themesmake-icons-wrapper .icon-search', function() {
            var text = $(this),
                value = this.value,
                at_customize_icons = text.closest( '.themesmake-icons-wrapper' ),
                icons_list_wrapper = at_customize_icons.find( '.icons-list-wrapper' );
