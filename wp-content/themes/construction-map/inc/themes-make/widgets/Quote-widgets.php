<?php
if (!class_exists('Construction_Map_Quote_Widget')) {
    class Construction_Map_Quote_Widget extends WP_Widget
    {
        private function defaults()
        {
            $defaults = array(
                'title' => 'Contact Us',
                'button-text1' => esc_html__('Contact Us', 'construction-map'),
                'button-text-link1' => '#',
                'image'=>'',
                'sub-title' => esc_html__('Please Contact Us if you required any helps to build houses', 'construction-map'),
            );
            return $defaults;
        }
        ...
        {
            parent::__construct(
                'construction-map-quote-widget',
                esc_html__('CM: Quotation Widget', 'construction-map'),
                array('description' => esc_html__('Construction Contact Section', 'construction-map'))
            );
        }
        public function form($instance)
        {
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'button-text1' ) ); ?>"><?php esc_html_e('Button Text1', 'construction-map'); ?></label><br/>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'button-text-link1' ) ); ?>">
                    <?php esc_html_e('Button Link1', 'construction-map'); ?>
                </label><br/>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('image')); ?>">
                    <?php esc_html_e('Image Size[1300 X 630]', 'construction-map'); ?>
                </label>
                <br/>
                <?php
                if (!empty($image)) :
...
add_action('widgets_init', 'construction_map_quote_widget');
function construction_map_quote_widget()
{
    register_widget('construction_map_Quote_Widget');
