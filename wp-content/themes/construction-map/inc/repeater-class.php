...
if (class_exists('WP_Customize_control')) {
    ...
    class Construction_Map_Repeater_Controler extends WP_Customize_Control
    {
        /**
         * The control type.
         *
        ...
        public $construction_map_box_label = '';

        public $construction_map_box_add_control = '';
        ...
        {
            $this->fields = $fields;
            $this->construction_map_box_label = $args['construction_map_box_label'];
            $this->construction_map_box_add_control = $args['construction_map_box_add_control'];
        ...
        <?php } ?>
            ...
            <ul class="construction_map-repeater-field-control-wrap">
                <?php
                $this->construction_map_get_fields();
                ?>
            </ul>

            <input type="hidden" <?php esc_attr($this->link()); ?> class="construction_map-repeater-collector"
                   value="<?php echo esc_attr($this->value()); ?>"/>
            <button type="button"
                    class="button construction_map-add-control-field"><?php echo esc_html($this->construction_map_box_add_control); ?></button>
            <?php
        }

        private function construction_map_get_fields()
        {
            $fields = $this->fields;
            $values = json_decode($this->value());
            ...
            if (is_array($values)) {
                foreach ($values as $value) {
                    ?>
                    <li class="construction_map-repeater-field-control">
                        <h3 class="construction_map-repeater-field-title"><?php echo esc_html($this->construction_map_box_label); ?></h3>

                        <div class="construction_map-repeater-fields">
                            <?php
                            foreach ($fields as $key => $field) {
                                $class = isset($field['class']) ? $field['class'] : '';
                                ?>
                                <div
                                    class="construction_map-fields construction_map-type-<?php echo esc_attr($field['type']) . ' ' . $class ?>">
                                    ...
                                    switch ($field['type']) {
                                        ...
                                        case 'upload':
                                            ...
                                            echo '<div class="construction_map-fields-wrap">';
                                            echo '<div class="attachment-media-view">';
                                            echo '<div class="placeholder' . $image_class . '">';
                                            esc_html__('No image selected', 'construction-map');
                                            echo '</div>';
                                            echo '<div class="thumbnail thumbnail-image">';
                                            echo $image;
                                            echo '</div>';
                                            echo '<div class="actions clearfix">';
                                            echo '<button type="button" class="button construction_map-delete-button align-left">' . esc_html__('Remove', 'construction-map') . '</button>';
                                            echo '<button type="button" class="button construction_map-upload-button alignright">' . esc_html__('Select Image', 'construction-map') . '</button>';
                                        ...
                                        case 'category':
                                            ...
                                            echo '<option value="0">' . esc_html__('Select Category', 'construction-map') . '</option>';
                                            echo '<option value="-1">' . esc_html__('Latest Posts', 'construction-map') . '</option>';
                                        ...
                                        case 'colorpicker':
                                            echo '<input data-default="' . esc_attr($default) . '" class="construction_map-color-picker" data-alpha="true" data-name="' . esc_attr($key) . '" type="text" value="' . esc_attr($new_value) . '"/>';
                                            break;
                                        ...
                                        case 'range':
                                            ...
                                            echo '<div class="construction_map-range-slider" >';
                                        ...
                                        case 'icon':
                                            echo '<div class="construction_map-selected-icon">';
                                            echo '<i class="' . esc_attr($new_value) . '"></i>';
                                            echo '<span><i class="fa fa-angle-down"></i></span>';
                                            echo '</div>';
                                            echo '<ul class="construction_map-icon-list clearfix">';
                                            $construction_map_font_awesome_icon_array = construction_map_font_awesome_icon_array();
                                            foreach ($construction_map_font_awesome_icon_array as $construction_map_font_awesome_icon) {
                                                $icon_class = $new_value == $construction_map_font_awesome_icon ? 'icon-active' : '';
                                                echo '<li class=' . $icon_class . '><i class="' . $construction_map_font_awesome_icon . '"></i></li>';
                                            }
                                            echo '</ul>';
                                            echo '<input data-default="' . esc_attr($default) . '" type="hidden" value="' . esc_attr($new_value) . '" data-name="' . esc_attr($key) . '"/>';
                                            break;
                                        ...
                                        case 'multicategory':
                                            ...
                                            echo '<ul class="construction_map-multi-category-list">';
                                            echo '<li><label><input type="checkbox" value="-1" ' . checked('-1', $new_value, false) . '/>' . esc_html_e('Latest Posts', 'construction-map') . '</label></li>';
                                            foreach ($this->cats as $cat) {
                            ...
                            <div class="clearfix construction_map-repeater-footer">
                                <div class="alignright">
                                    <a class="construction_map-repeater-field-remove"
                                       href="#remove"><?php esc_html_e('Delete', 'construction-map') ?></a> |
                                    <a class="construction_map-repeater-field-close"
                                       href="#close"><?php esc_html_e('Close', 'construction-map') ?></a>
                                </div>
                            </div>
                        </div>
                    </li>
...
if (!function_exists('construction_map_font_awesome_icon_array')) {
    function construction_map_font_awesome_icon_array()
    {
