...
if ( ! class_exists( 'Fire_Blog_About' ) ) {
	...
	class Fire_Blog_About {
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Fire_Blog_About ) ) {
				self::$instance = new Fire_Blog_About;
		...
		public function setup_config() {
			...
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'fire-blog' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'fire-blog' ), $this->theme_name ) . '</a><a href="' . esc_url( admin_url() . 'themes.php?status=fire_blog_hide_msg' ) . '" class="button" style="margin-left:5px;">' . esc_html__( 'Hide this notice', 'fire-blog' ) . '</a></p>' );
		}
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Fire Blog','fire-blog' ) . '</th>';
		            echo '<th>' . esc_html__( 'Fire Blog PRO','fire-blog' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="' . esc_url( 'https://cyclonethemes.com/downloads/fireblog-pro/' ) . '" target="_blank" class="button button-primary button-hero">Fire Blog PRO</a></td>';
		            echo '</tr>';
