...
function hybrid_archive_title_filter( $title ) {
	...
	elseif ( is_category() )
		$title = str_replace( __( 'Category: ', 'metrolo' ), '', $title );

	elseif ( is_tag() )
		$title = str_replace( __( 'Tag: ', 'metrolo' ), '', $title );

	elseif ( is_tax() && !is_tax( 'post_format' ) )
		$title = single_term_title( '', false );

	elseif ( is_author() )
		$title = str_replace( __( 'Author: ', 'metrolo' ), '', $title );

	elseif ( is_search() )
		/* Translators: %s is search query */
		$title = sprintf( esc_html__( 'Search results for: %s', 'metrolo' ), get_search_query() );

	elseif ( is_post_type_archive() )
		$title = str_replace( __( 'Archives: ', 'metrolo' ), '', $title );
