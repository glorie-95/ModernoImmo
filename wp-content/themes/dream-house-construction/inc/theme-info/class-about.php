...
if ( ! class_exists( 'Dream_House_Construction_About' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Dream_House_Construction_About {
		...
		/**
		 * Singleton instance of Fire_Blog_About.
		 *
		 * @var Fire_Blog_About $instance Fire_Blog_About instance.
		 */
		private static $instance;

		/**
		 * Main Fire_Blog_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Dream_House_Construction_About ) ) {
				self::$instance = new Dream_House_Construction_About;
		...
		public function setup_config() {
			...
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'dream-house-construction' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'dream-house-construction' ), $this->theme_name ) . '</a><a href="' . esc_url( admin_url() . 'themes.php?status=fire_blog_hide_msg' ) . '" class="button" style="margin-left:5px;">' . esc_html__( 'Hide this notice', 'dream-house-construction' ) . '</a></p>' );
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'dream-house-construction' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'dream-house-construction' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'dream-house-construction' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'dream-house-construction' );
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Fire Blog','dream-house-construction' ) . '</th>';
		            echo '<th>' . esc_html__( 'Fire Blog PRO','dream-house-construction' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="' . esc_url( 'https://www.templatesell.com/item/fireblog-pro/' ) . '" target="_blank" class="button button-primary button-hero">Fire Blog PRO</a></td>';
		            echo '</tr>';
